...
class TestBfSanity:
    ...
    def test_check_config_db_json(self):
        ...
        # TODO: restore validation below once DPU will support all values
    ...
    def test_show_commands(self):
        ...
        with allure.step("Verify platform summary output"):
    ...
    def check_interfaces_output(ports_list, output, expected_template):
        ...
        for port, port_details in parsed_output.items():
            for template_key, template_regex in expected_template.items():
                assert re.search(template_regex, port_details[template_key]) is not None, \
                    f"The current value: {port_details[template_key]} can't be matched by regex(str): {template_regex}"
    ...
    def check_interfaces_counters_detailed_output(self):
        ...
        for template_regex in INTERFACE_COUNTERS_DETAILED_TEMPLATE:
            assert re.search(templ_regex, output) is not None, \
                f"Not found the regex {template_regex} in the output of" \
