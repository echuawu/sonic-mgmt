...
def test_rj45_cli(engines, cli_objects, platform_params, rj45_ports_list, sfp_ports_list, dut_ports_interconnects,
    ...
    with allure.step('Get hwsku data'):
        hwsku_data = cli_objects.dut.general.get_hwsku_json_as_dict(engines.dut, platform_params.platform,
    ...
    with allure.step('Set test ports speed'):
        for port in test_ifaces_list:
            cli_objects.dut.interface.set_interface_speed(engines.dut, port, rj45_port_speed)
            cleanup_list.append((cli_objects.dut.interface.set_interface_speed,
                                 (engines.dut, port, '1000M')))

        cli_objects.dut.interface.set_interface_speed(engines.dut, sfp_port, '1000')
        cleanup_list.append((cli_objects.dut.interface.set_interface_speed,
                             (engines.dut, sfp_port, sfp_port_orig_speed)))
    ...
    with allure.step('Create 2 LAG interfaces with 2 physical members each'):
        ...
        for lag_config_dict in lag_configs_list:
            cli_objects.dut.lag.create_lag_interface_and_assign_physical_ports(engines.dut, lag_config_dict)
            cleanup_list.append((cli_objects.dut.lag.delete_lag_interface_and_unbind_physical_ports,
                                 (engines.dut, lag_config_dict)))

    with allure.step('Validate that SFP and RJ45 ports cannot be members of the same LAG'):
        add_sfp_port_output = cli_objects.dut.lag.add_port_to_port_channel(engines.dut, sfp_port, LACP_IFACES_LIST[0])
...
def get_cli_outputs(engine, cli_object):
    with allure.step('Get interfaces status'):
        interfaces_status = cli_object.interface.parse_interfaces_status(engine)

    with allure.step('Get transceiver eeprom'):
        transceiver_eeprom = cli_object.interface.parse_interfaces_transceiver_eeprom(engine)

    with allure.step('Get transceiver error-status'):
        transceiver_error_status = cli_object.interface.parse_interfaces_transceiver_error_status(engine)

    with allure.step('Get transceiver lpmode'):
        transceiver_lpmode = cli_object.interface.parse_interfaces_transceiver_lpmode(engine)

    with allure.step('Get transceiver presence'):
        transceiver_presence = cli_object.interface.parse_interfaces_transceiver_presence(engine)
