...
def test_auto_neg(topology_obj, engines, cli_objects, tested_lb_dict,
    ...
    logger.info("Enable auto-negotiation with default settings and validate speed/type configuration is as expected.")
    ...
    logger.info("Enable auto-negotiation with custom settings and validate speed/type configuration is as expected.")
    auto_neg_checker(topology_obj, engines.dut, cli_objects.dut, tested_lb_dict, conf,
                     cable_type_to_speed_capabilities_dict, cleanup_list)
...
def get_speeds_in_Gb_str_format(speeds_list):
    ...
    sorted(speeds_in_str_format, key=speed_string_to_int)
    ...
    with allure.step('creating custom configuration'):
        ...
        for split_mode, lb_list in tested_lb_dict.items():
            for lb in lb_list:
                ...
                mutual_speed_in_subset = random.choice(lb_mutual_speeds)
                all_adv_speeds = []
                all_adv_types = []
                for port in lb:
                    adv_speeds = set(random.choices(lb_mutual_speeds, k=random.choice(range(1, len(lb_mutual_speeds)))))
...
def configure_ports(engine, cli_object, conf, base_interfaces_speeds, cable_type_to_speed_capabilities_dict,
    ...
    with allure.step('Configuring speed, advertised speed, type and advertised type on ports: {}'
        ...
        for port, port_conf_dict in conf.items():
            ...
            base_type = get_interface_cable_type(get_matched_types([base_speed], cable_type_to_speed_capabilities_dict).pop())
            ...
            if port_conf_dict[AutonegCommandConstants.ADV_SPEED] != 'all':
            ...
            if port_conf_dict[AutonegCommandConstants.ADV_TYPES] != 'all':
                cleanup_list.append((cli_object.interface.config_advertised_interface_type, (engine, port, 'all')))
            cli_object.interface.\
...
def auto_neg_toggle_peer_checker(topology_obj, engines, cli_objects, conf, interfaces, cable_type_to_speed_capabilities_dict, cleanup_list):
    ...
    configure_ports(engines.ha, cli_objects.ha, conf={interfaces.ha_dut_1: conf[interfaces.ha_dut_1]},
                    ...
                    cable_type_to_speed_capabilities_dict=cable_type_to_speed_capabilities_dict,
                    cleanup_list=cleanup_list)
    logger.info("Disable the dut interface {}".format(interfaces.dut_ha_1))
    disable_interface(engines.dut, cli_objects.dut, interfaces.dut_ha_1, cleanup_list)
    ...
    logger.info("Enable the dut interface {}".format(interfaces.dut_ha_1))
    cli_objects.dut.interface.enable_interface(engines.dut, interfaces.dut_ha_1)
    ...
    logger.info("Toggle host port {}".format(interfaces.ha_dut_1))
    disable_interface(engines.ha, cli_objects.ha, interfaces.ha_dut_1, cleanup_list)
    cli_objects.ha.interface.enable_interface(engines.ha, interfaces.ha_dut_1)
    for port, port_conf_dict in conf.items():
