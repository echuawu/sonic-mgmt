...
class LinuxLagLacpCli(LagLacpCliCommon):
    ...
    def create_lag_interface_and_assign_physical_ports(self, lag_lacp_info):
        ...
        LinuxInterfaceCli(engine=self.engine).add_interface(lag_lacp_iface_name, iface_type='bond')

        if lag_type == 'lag':
            pass
        elif lag_type == 'lacp':
            LinuxLagLacpCli(engine=self.engine).set_bond_mode(lag_lacp_iface_name, bond_mode='4')
        else:
        ...
        for member_port in lag_lacp_info['members']:
            LinuxInterfaceCli(engine=self.engine).disable_interface(member_port)
            LinuxInterfaceCli(engine=self.engine).add_port_to_bond(member_port, lag_lacp_iface_name)
            LinuxInterfaceCli(engine=self.engine).enable_interface(member_port)

        LinuxInterfaceCli(engine=self.engine).enable_interface(lag_lacp_iface_name)
    ...
    def delete_lag_interface_and_unbind_physical_ports(self, lag_lacp_info):
        ...
        """
        lag_lacp_iface_name = lag_lacp_info['name']
        LinuxInterfaceCli(engine=self.engine).del_interface(lag_lacp_iface_name)
        for member_port in lag_lacp_info['members']:
            LinuxInterfaceCli(engine=self.engine).enable_interface(member_port)
