...
class LinuxVxlanCli(VxlanCliCommon):
    ...
    def configure_vxlan(engine, vxlan_info):
        ...
        LinuxVxlanCli.add_vtep(engine, vtep_name, vxlan_info['vtep_src_ip'], vxlan_info['vtep_dst_ip'],
                               vxlan_info['vni'])
        ...
        for ip_mask in vxlan_info.get('vtep_ips', []):
            ip = ip_mask[0]
            mask = ip_mask[1]
            LinuxIpCli.add_ip_to_interface(engine, vtep_name, ip, mask)
    ...
    @staticmethod
    def add_vtep(engine, vtep_name, src_ip, dst_ip, vni):
        """
        ...
        :return: command output
        """
        return engine.run_cmd('ip link add {} type vxlan id {} dstport 4789 local {} remote {}'.format(
            vtep_name, vni, src_ip, dst_ip))
