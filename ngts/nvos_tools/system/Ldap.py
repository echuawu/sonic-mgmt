...
class Ldap(BaseComponent):
    ...
    def set_hostname(self, hostname, apply=False, ask_for_confirmation=False):
        return self.set("hostname {}".format(hostname), apply=apply, ask_for_confirmation=ask_for_confirmation)
    ...
    def set_bind_dn(self, user, apply=False, ask_for_confirmation=False):
        return self.set("bin-dn {}".format(user), apply=apply, ask_for_confirmation=ask_for_confirmation)

    def set_bind_password(self, password, apply=False, ask_for_confirmation=False):
        return self.set("bind-password {}".format(password), apply=apply, ask_for_confirmation=ask_for_confirmation)

    def set_port(self, port, apply=False, ask_for_confirmation=False):
        return self.set("port {}".format(port), apply=apply, ask_for_confirmation=ask_for_confirmation)

    def set_base_dn(self, base, apply=False, ask_for_confirmation=False):
        return self.set("base-dn {}".format(base), apply=apply, ask_for_confirmation=ask_for_confirmation)

    def set_login_attribute(self, login_attr, apply=False, ask_for_confirmation=False):
        return self.set("login-attribute {}".format(login_attr), apply=apply, ask_for_confirmation=ask_for_confirmation)

    def set_group_attribute(self, group_attr, apply=False, ask_for_confirmation=False):
        return self.set("group-attribute {}".format(group_attr), apply=apply, ask_for_confirmation=ask_for_confirmation)

    def set_scope(self, scope, apply=False, ask_for_confirmation=False):
        return self.set("scope {}".format(scope), apply=apply, ask_for_confirmation=ask_for_confirmation)

    def set_version(self, version, apply=False, ask_for_confirmation=False):
        return self.set("version {}".format(version), apply=apply, ask_for_confirmation=ask_for_confirmation)

    def set_timeout_bind(self, timeout, apply=False, ask_for_confirmation=False):
        return self.set("timeout-bind {}".format(timeout), apply=apply, ask_for_confirmation=ask_for_confirmation)

    def set_timeout_search(self, timeout, apply=False, ask_for_confirmation=False):
        return self.set("timeout-search {}".format(timeout), apply=apply, ask_for_confirmation=ask_for_confirmation)
