...
def get_platform_type(item):
    ...
    if not platform_type:
        setup_name = item.session.config.option.setup_name
        topology_obj = get_topology_by_setup_name(setup_name, slow_cli=False)
        platform_type = get_platform_info(topology_obj).get('platform')
...
def prepare_checkers(skip_dict, current_platform):
    ...
    for skip_by in skip_dict:
        ...
        try:
            skip_module = __import__(skip_by)
