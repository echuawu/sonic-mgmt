...
def test_adjust_switch_for_community_tests(topology_obj):
    ...
    with allure.step("Rename original show script to show.orig"):
        ...
        engine.run_cmd("sudo cp {origin} {new_name}".format(origin=new_show_path, new_name=origin_show_path))

    with allure.step("Create new show script"):
        engine.run_cmd("sudo rm {}".format(new_show_path))
        line_to_write = '{origin_path} $1 $2 $3 | tail -n +3'.format(origin_path=origin_show_path)
        engine.run_cmd("sudo touch {origin_path}".format(origin_path=new_show_path))
        engine.run_cmd("sudo chmod 777 {origin_path}".format(origin_path=new_show_path))
        engine.run_cmd("sudo echo \'{line}\' >> {file}".format(line=line_to_write, file=new_show_path))
