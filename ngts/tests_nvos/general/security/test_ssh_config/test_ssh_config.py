...
from ngts.tests_nvos.general.security.conftest import create_ssh_login_engine, ssh_to_device_and_retrieve_raw_login_ssh_notification
...
def test_ssh_config_good_flow(engines):
    ...
    with allure.step("Validating login authentication-retries"):
        ...
        system.ssh_server.set(SshConfigConsts.AUTH_RETRIES, auth_retries, apply=True, ask_for_confirmation=True).verify_result()
        with allure.step("Failing to Connect {} times to get logged out of session".format(auth_retries)):
            ...
            try:
                ...
                for iteration in range(auth_retries - 1):
                    ...
                    logger.info(
                        "Itertation {} - connecting using random password: {} for user: {}".format(iteration,
                                                                                                   random_password,
                                                                                                   DefaultConnectionValues.ADMIN))
    ...
    with allure.step("Validating ssh login ports"):
        with allure.step("validating ssh login ports, in range [{}-{}]".format(SshConfigConsts.MIN_LOGIN_PORT, SshConfigConsts.MAX_LOGIN_PORT)):
            logger.info("validating ssh login ports, in range [{}-{}]".format(SshConfigConsts.MIN_LOGIN_PORT, SshConfigConsts.MAX_LOGIN_PORT))
            ...
            ssh_to_device_and_retrieve_raw_login_ssh_notification(engines.dut.ip, username=DefaultConnectionValues.ADMIN,
    ...
    with allure.step("Validating login timeout"):
        ...
        system.ssh_server.set(SshConfigConsts.LOGIN_TIMEOUT, login_timeout, apply=True, ask_for_confirmation=True).verify_result()
        try:
