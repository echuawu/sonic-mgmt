...
def test_gnmi_basic_flow_poll(engines, devices):
    ...
    """
    gnmi_basic_flow(engines, devices.dut.switch_type.lower(), mode=GnmiMode.POLL)
    ...
    Check gnmi basic flow: show command , disable and enable commands, validate stream updates to gnmi-client,
     with subscribe mode - once.
        Test flow:
            ...
            11. validate gnmi-server stream updates
    """
    gnmi_basic_flow(engines, devices.dut.switch_type.lower(), mode=GnmiMode.ONCE)
...
def test_gnmi_basic_flow_stream(test_api, engines, devices):
    ...
    gnmi_basic_flow(engines, devices.dut.switch_type.lower(), mode=GnmiMode.STREAM)
...
def test_gnmi_bad_flow(test_api, engines, devices):
    ...
    with allure_step("Subscribe to the gnmi server for data that is not supported"):
        name = 'sw1p1s1' if switch_type == 'nvl' else 'sw1p1'
        xpath = f'interfaces/interface[name={name}]/state/counters/in-broadcast-pkts'
