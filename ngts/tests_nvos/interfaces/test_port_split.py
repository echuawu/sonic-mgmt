...
def test_ib_split_port_no_breakout_profile(engines, interfaces, start_sm):
    ...
    with allure.step("Try split splitter port in not breakout system profile"):
        ...
        for port in split_ports:
            port.ib_interface.link.breakout.set(value=IbInterfaceConsts.LINK_BREAKOUT_HDR, apply=True,
                                                ask_for_confirmation=True).verify_result(False)
            port.ib_interface.link.breakout.set(value=IbInterfaceConsts.LINK_BREAKOUT_NDR, apply=True,
                                                ask_for_confirmation=True).verify_result(False)
    ...
    with allure.step("Try split eth0 and ib0 port in not breakout system profile"):
        ...
        mgmt_port.interface.link.breakout.set(value=IbInterfaceConsts.LINK_BREAKOUT_HDR, apply=True,
                                              ask_for_confirmation=True).verify_result(False)
        mgmt_port.interface.link.breakout.set(value=IbInterfaceConsts.LINK_BREAKOUT_NDR, apply=True,
                                              ask_for_confirmation=True).verify_result(False)
        NvueGeneralCli.detach_config(TestToolkit.engines.dut)
        output = NvueGeneralCli.diff_config(TestToolkit.engines.dut)
        assert not output, "config not dettached"
        ...
        ipoib_port.interface.link.breakout.set(value=IbInterfaceConsts.LINK_BREAKOUT_HDR, apply=True,
                                               ask_for_confirmation=True).verify_result(False)
        ipoib_port.interface.link.breakout.set(value=IbInterfaceConsts.LINK_BREAKOUT_NDR, apply=True,
                                               ask_for_confirmation=True).verify_result(False)
        NvueGeneralCli.detach_config(TestToolkit.engines.dut)
        output = NvueGeneralCli.diff_config(TestToolkit.engines.dut)
        assert not output, "config not dettached"
    ...
    with allure.step("Try split already splitted port"):
        ...
        for child_port in child_ports:
            child_port.ib_interface.link.breakout.set(value=IbInterfaceConsts.LINK_BREAKOUT_HDR, apply=True,
                                                      ask_for_confirmation=True).verify_result(False)
