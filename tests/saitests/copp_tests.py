...
class ControlPlaneBaseTest(BaseTest):
    ...
    def copp_test(self, packet, count, send_intf, recv_intf):
        ...
        if self.verbose:
            ...
            print()
            print(("Sent through NN to local ptf_nn_agent:    ",
                  e_c_0[1] - b_c_0[1]))
            print(("Sent through If to remote ptf_nn_agent:   ",
                  e_n_0[1] - b_n_0[1]))
            print(("Recv from If on remote ptf_nn_agent:      ",
                  e_c_1[0] - b_c_1[0]))
            print(("Recv from NN on from remote ptf_nn_agent: ",
                  e_n_1[0] - b_n_1[0]))
    ...
    def one_port_test(self, port_number):
        ...
        total_rcv_pkt_cnt, time_delta, time_delta_ms, tx_pps, rx_pps = self.copp_test(
            str(packet), self.PKT_TX_COUNT, (0, port_number), (1, port_number))
        self.printStats(self.PKT_TX_COUNT, total_rcv_pkt_cnt,
                        time_delta, tx_pps, rx_pps)
