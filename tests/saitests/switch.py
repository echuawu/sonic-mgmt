...
def sai_thrift_create_router_interface(client, vr_id, is_port, port_id, vlan_id, v4_enabled, v6_enabled, mac):
    ...
    if is_port:
        ...
        rif_attribute2_value = sai_thrift_attribute_value_t(
            s32=SAI_ROUTER_INTERFACE_TYPE_PORT)
...
def sai_thrift_create_stp_entry(client, vlan_list):
    vlanlist = sai_thrift_vlan_list_t(
        vlan_count=len(vlan_list), vlan_list=vlan_list)
...
def sai_thrift_create_acl_entry(client, acl_table_id,
    ...
    attribute_value = sai_thrift_attribute_value_t(
        aclfield=sai_thrift_acl_field_data_t(data=sai_thrift_acl_data_t(oid=acl_table_id)))
    ...
    # Priority
    attribute_value = sai_thrift_attribute_value_t(
        aclfield=sai_thrift_acl_field_data_t(data=sai_thrift_acl_data_t(u32=10)))
    ...
    if in_port_list:
        acl_port_list = sai_thrift_object_list_t(
            count=len(in_port_list), object_id_list=in_port_list)
        attribute_value = sai_thrift_attribute_value_t(aclfield=sai_thrift_acl_field_data_t(
            data=sai_thrift_acl_data_t(objlist=acl_port_list)))
    ...
    if out_port_list:
        acl_port_list = sai_thrift_object_list_t(
            count=len(out_port_list), object_id_list=out_port_list)
        attribute_value = sai_thrift_attribute_value_t(aclfield=sai_thrift_acl_field_data_t(
            data=sai_thrift_acl_data_t(objlist=acl_port_list)))
    ...
    if action == 1:
        ...
        if ingress_mirror is not None:
            ...
            attribute = sai_thrift_attribute_t(
                id=SAI_ACL_ENTRY_ATTR_ACTION_MIRROR_EGRESS, value=attribute_value)
            acl_attr_list.append(attribute)
...
def sai_thrift_create_scheduler_profile(client, max_rate, algorithm=0):
    ...
    attribute = sai_thrift_attribute_t(id=SAI_SCHEDULER_ATTR_SCHEDULING_TYPE,
                                       value=attribute_value)
    scheduler_attr_list.append(attribute)
    scheduler_profile_id = client.sai_thrift_create_scheduler_profile(
        scheduler_attr_list)
...
def sai_thrift_create_buffer_profile(client, pool_id, size, threshold, xoff_th, xon_th):
    ...
    buffer_profile_id = client.sai_thrift_create_buffer_profile(
        buffer_attr_list)
...
def sai_thrift_read_port_watermarks(client, port):
    ...
    for queue in queue_list[:8]:
        thrift_results = client.sai_thrift_get_queue_stats(
            queue, q_wm_ids, len(q_wm_ids))
        queue_res.append(thrift_results[0])

    for pg in pg_list:
        thrift_results = client.sai_thrift_get_pg_stats(
            pg, pg_wm_ids, len(pg_wm_ids))
...
def sai_thrift_read_buffer_pool_watermark(client, buffer_pool_id):
    ...
    if not wm_vals:
        print("sai_thrift_read_buffer_pool_watermark returns empty list",
              file=sys.stderr)
        return None
    return wm_vals[0]
...
def sai_thrift_read_headroom_pool_watermark(client, buffer_pool_id):
    ...
    wm_vals = client.sai_thrift_get_buffer_pool_stats(
        buffer_pool_id, buffer_pool_wm_ids)
    if not wm_vals:
        print("sai_thrift_read_headroom_pool_watermark returns empty list",
              file=sys.stderr)
        return None
    return wm_vals[0]
