...
class LogAnalyzer:
    def __init__(self, ansible_host, marker_prefix, request=None, dut_run_dir="/tmp", start_marker=None, additional_files={}):
        self.ansible_host = ansible_host
    ...
    def analyze(self, marker, fail=True, maximum_log_length=None, store_la_logs=False):
        ...
        for key, value in list(analyzer_parse_result.items()):
            ...
            analyzer_summary["match_files"][key] = {"match": len(matching_lines),
                                                    "expected_match": len(expecting_lines)}
