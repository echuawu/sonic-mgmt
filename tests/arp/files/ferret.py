...
Record = namedtuple(
    'Record', ['hostname', 'family', 'expired', 'lo', 'mac', 'vxlan_id'])
...
class Ferret(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def generate_entries(self, hostname, family, expire, lo, info, mapping_family):
        for i in info['vlanInterfaces']:
            ...
            for j in i[mapping_family]:
                ...
                addr = str(j['ipAddr'])
                r = Record(hostname=hostname, family=family,
                           expired=expire, lo=lo, mac=mac, vxlan_id=vxlan_id)
                self.db[addr] = r
    ...
    def update_db(self, info):
        ...
        self.generate_entries(hostname, 'ipv4', expired,
                              lo_ipv4, info, 'ipv4AddrMappings')
        self.generate_entries(hostname, 'ipv6', expired,
                              lo_ipv6, info, 'ipv6AddrMappings')
    ...
    def send_resp(self, info):
        ...
        f, line = self.generate_response(result)
...
class RestAPI(object):
    ...
    def __init__(self, obj, db, src_ip):
        ...
        self.context.load_cert_chain(
            certfile="/opt/test.pem", keyfile="/opt/test.key")
    ...
    def __init__(self, iface, bpf_src):
        ...
        if bpf_src is not None:
            ...
            self.socket.setsockopt(
                socket.SOL_SOCKET, self.SO_ATTACH_FILTER, bpf)
...
class Poller(object):
    def __init__(self, httpd, interfaces, responder):
        ...
        self.mapping = {
            interface.handler(): interface for interface in interfaces}
        self.httpd = httpd
...
class Responder(object):
    ...
    def action(self, interface):
        ...
        if r.family == 'ipv4':
            ...
            if self.vxlan_port:
                # udp
                new_pkt += binascii.unhexlify('c000%04x004c1280' %
                                              self.vxlan_port)
            else:
                new_pkt += binascii.unhexlify('c00012b5004c1280')  # udp
            new_pkt += binascii.unhexlify('08000000%06x00' %
                                          r.vxlan_id)  # vxlan

            arp_reply = self.generate_arp_reply(binascii.unhexlify(
...
def parse_args():
    ...
    parser.add_argument('-f', '--config-file',
                        help='file with configuration', required=True)
    parser.add_argument(
        '-s', '--src-ip', help='Ferret endpoint ip', required=True)
    parser.add_argument('-a', '--asic-type',
                        help='ASIC vendor name', type=str, required=False)
    parser.add_argument('-p', '--vxlan-port', help='VXLAN port',
