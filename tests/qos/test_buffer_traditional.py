...
def load_lossless_info_from_pg_profile_lookup(duthost):
    ...
    threshold_mode = duthost.shell(
        'redis-cli -n 4 hget "BUFFER_POOL|ingress_lossless_pool" mode')['stdout']
...
def test_buffer_pg(duthosts, rand_one_dut_hostname, conn_graph_facts):
    ...
    def _check_port_buffer_info_and_get_profile_oid(duthost, port, expected_profile, use_assert=True):
        ...
        """
        profile_in_pg = duthost.shell(
            'redis-cli -n 4 hget "BUFFER_PG|{}|3-4" profile'.format(port))['stdout']
        buffer_profile_oid = None
        default_lossless_pgs = ['3', '4']

        if expected_profile:
            if not _check_condition(profile_in_pg == expected_profile,
            ...
            if pg_name_map:
                for pg in default_lossless_pgs:
                    ...
                    buffer_pg_asic_key = duthost.shell(
                        'redis-cli -n 1 keys *{}*'.format(buffer_pg_asic_oid))['stdout']
                    buffer_profile_oid_in_pg = duthost.shell(
                        'redis-cli -n 1 hget {} SAI_INGRESS_PRIORITY_GROUP_ATTR_BUFFER_PROFILE'.
                        format(buffer_pg_asic_key))['stdout']
            ...
            if pg_name_map:
                for pg in default_lossless_pgs:
                    ...
                    buffer_profile_oid_in_pg = duthost.shell(
                        ...
                        .format(buffer_pg_asic_key))['stdout']
                    logging.info(
                        "Checking admin-down port {} lossless PG {}".format(port, pg))
    ...
    def _check_port_buffer_info_and_return(duthost, port, expected_profile):
        ...
        _, result = _check_port_buffer_info_and_get_profile_oid(
            duthost, port, expected_profile, False)
        return result
    ...
    pg_name_map = make_dict_from_output_lines(duthost.shell(
        'redis-cli -n 2 hgetall COUNTERS_PG_NAME_MAP')['stdout'].split())
    cable_length_map = make_dict_from_output_lines(duthost.shell(
    ...
    for port in configdb_ports:
        port_config = make_dict_from_output_lines(duthost.shell(
            'redis-cli -n 4 hgetall "PORT|{}"'.format(port))['stdout'].split())
        ...
        if is_port_up or not RECLAIM_BUFFER_ON_ADMIN_DOWN:
            ...
            buffer_profile_oid, _ = _check_port_buffer_info_and_get_profile_oid(
                duthost, port, expected_profile)

            if expected_profile not in profiles_checked:
                profile_info = make_dict_from_output_lines(duthost.shell(
                    'redis-cli -n 4 hgetall "{}"'.format(expected_profile[1:-1]))['stdout'].split())
                pytest_assert(profile_info == DEFAULT_LOSSLESS_PROFILES[(speed, cable_length)],
                              ...
                              .format(expected_profile, profile_info,
                                      DEFAULT_LOSSLESS_PROFILES[(speed, cable_length)]))
                ...
                if buffer_profile_oid:
                    ...
                    buffer_profile_key = duthost.shell(
                        'redis-cli -n 1 keys *{}*'.format(buffer_profile_oid))['stdout']
                    buffer_profile_asic_info = make_dict_from_output_lines(duthost.shell(
                        'redis-cli -n 1 hgetall {}'.format(buffer_profile_key))['stdout'].split())
                    pytest_assert(
                        buffer_profile_asic_info['SAI_BUFFER_PROFILE_ATTR_XON_TH'] ==
                        profile_info['xon'] and
                        buffer_profile_asic_info['SAI_BUFFER_PROFILE_ATTR_XOFF_TH'] ==
                        profile_info['xoff'] and
            ...
            logging.info(
                "Checking admin-down port buffer information: {}".format(port))
            _, _ = _check_port_buffer_info_and_get_profile_oid(
                duthost, port, None)

    port_to_shutdown = admin_up_ports.pop()
    expected_profile = duthost.shell(
        'redis-cli -n 4 hget "BUFFER_PG|{}|3-4" profile'.format(port_to_shutdown))['stdout']
    try:
        ...
        wait_until(60, 5, _check_port_buffer_info_and_return,
                   duthost, port_to_shutdown, expected_profile_admin_down)
