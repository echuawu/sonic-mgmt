...
from tests.common.dualtor.dual_tor_utils import upper_tor_host, lower_tor_host, dualtor_info  # noqa F401
...
def config_counter_poll_interval(upper_tor_host, lower_tor_host):  # noqa F811
    """
    Fixture to set the counterpoll intervals to 1s during the test, so we can save some time.
    """
    origin_queue_interval = upper_tor_host.get_counter_poll_status()['QUEUE_STAT']['interval']
    upper_tor_host.set_counter_poll_interval('QUEUE_STAT', 1000)
    lower_tor_host.set_counter_poll_interval('QUEUE_STAT', 1000)
    yield
    upper_tor_host.set_counter_poll_interval('QUEUE_STAT', origin_queue_interval)
    lower_tor_host.set_counter_poll_interval('QUEUE_STAT', origin_queue_interval)


def test_active_tor_normal_traffic(ptfhost, rand_selected_dut, rand_unselected_dut, upper_tor_host, # noqa F811
                                   lower_tor_host, toggle_all_simulator_ports_to_rand_selected_tor, # noqa F811
                                   tbinfo, ptfadapter):
    """
...
def test_traffic_between_servers(ptfhost, upper_tor_host, lower_tor_host, # noqa F811
    ...
    with allure.step("Prepare test parameters and switch one server on the lower tor to active"):
        ...
        toggle_simulator_port_to_lower_tor(lower_tor_active_interface)
    ...
    with allure.step("Start the traffic test"):
        for dscp, outer_dscp, queue in dscp_to_queue_mapping:
            ...
            # Clear queue counters before sending traffic
            lower_tor_host.shell('sonic-clear queuecounters')
            ptfadapter.dataplane.flush()
            ...
            pytest_assert(wait_until(
                10, 2, 0, check_queue_counter, lower_tor_host,
