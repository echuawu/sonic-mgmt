...
def get_memory_sizes(module):
    _, out, _ = exec_command(module, cmd="free -m",
                             msg="checking memory total/free sizes")
    lines = out.split('\n')
    if len(lines) < 2:
        return -1, -1
...
def setup_swap_if_necessary(module):
    ...
    if total < 2048 or avail < 1200:
        ...
        exec_command(
            ...
            .format('/host/swapfile'), msg="Create a temporary swap file")
...
def reduce_installed_sonic_images(module):
    ...
    if curr_image != next_image:
        exec_command(
            module, cmd="sonic_installer set-next-boot {}".format(curr_image), ignore_error=True)
...
def download_new_sonic_image(module, new_image_url, save_as):
    ...
    if path.exists(save_as):
        _, out, _ = exec_command(
            module, cmd="sonic_installer binary_version {}".format(save_as))
