...
git clone -b ${LOCAL_BRANCH} "ssh://git-nbu-sw.nvidia.com:12024/switchx/sonic/sonic-mgmt" -b ${LOCAL_BRANCH} ${tmp_dir}
cd ${tmp_dir} || exit 1
FILE=".git/hooks/commit-msg"
if [[ ! -f "$FILE" ]]; then
    echo "$FILE does not exists. copying commit message"
    scp -p -P 12024 git-nbu-sw.nvidia.com:hooks/commit-msg ".git/hooks/"
fi
